...
test("buildWorkflowArtBundle exposes selected params and bakes the rest into with", () => {
  ...
  assert.equal(
    workflow.nodes.find((node) => node.id === "b")?.with.image,
    "${{ nodes.a.outputs.output_image }}",
  );
...
test("buildWorkflowArtBundle preserves target ports for multi-image edges", () => {
  const snapshot: HookCanvasSnapshot = {
    ...
    nodes: [
      ...
      artNode(
        "raw-color",
        "color",
        "neuro.official/custom-1770131241684",
  ...
  assert.deepEqual(compress?.with, {
    input: "${{ nodes.color.outputs.output }}",
  });
