...
import { DEFAULT_ARTLOOM_COMPAT_SETTINGS } from "./loomApi.ts";

test("uses dark as the product default while retaining explicit system support", () => {
  assert.equal(DEFAULT_ARTLOOM_COMPAT_SETTINGS.appearance_version, 1);
  assert.equal(DEFAULT_ARTLOOM_COMPAT_SETTINGS.general.theme, "dark");
  assert.equal(DEFAULT_ARTLOOM_COMPAT_SETTINGS.hook_general.theme, "dark");
});
