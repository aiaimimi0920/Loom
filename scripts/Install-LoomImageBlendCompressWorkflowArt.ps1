...
if ($ArtId -ne "custom-image-blend-compress-workflow") {
    throw "The pluginized workflow package has a fixed manifest id; install a third-party package for custom ids."
}
$installer = Join-Path $PSScriptRoot "Install-LoomSampleArtPackage.ps1"
$arguments = @("-PackageName", "image-blend-compress", "-BaseUrl", $BaseUrl, "-InstallMode", $InstallMode)
