...
Assert-Workflow -Name "build-windows.yml" -RequiredText @(
    ...
    'Swatinem/rust-cache@6323deb102c322ba6fcbdcafc7e3dddab59af2b6',
    '.\scripts\Invoke-CiCommand.ps1',
    'Standalone Loom build failed',
    'Standalone Loom verification failed',
...
Assert-Workflow -Name "release-tag.yml" -RequiredText @(
    ...
    "'^V\d+\.\d+\.\d+$'",
    '.\scripts\Invoke-CiCommand.ps1',
    'Tagged Loom build failed',
    'Tagged Loom verification failed',
    '.\scripts\build-release.ps1',
    '.\scripts\verify-release.ps1',
    '-RunSmoke',
    '-RequireCleanSource',
    ...
    'Loom-Plugin-SDK-${{ env.LOOM_TAG }}-windows-x64.zip.sha256'
)
...
foreach ($hiddenAsset in @('Loom-CLI-', 'sbom/', 'provenance/', 'manifest.json', 'checksums.sha256')) {
