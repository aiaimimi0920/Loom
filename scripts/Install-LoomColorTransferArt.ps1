...
if ($ArtId -ne "custom-1770131241684") {
    throw "The pluginized color-transfer package has a fixed manifest id; install a third-party package for custom ids."
}
$installer = Join-Path $PSScriptRoot "Install-LoomSampleArtPackage.ps1"
$arguments = @("-PackageName", "color-transfer", "-BaseUrl", $BaseUrl, "-InstallMode", $InstallMode)
