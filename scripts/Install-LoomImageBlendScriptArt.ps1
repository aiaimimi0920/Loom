...
if ($ArtId -ne "custom-image-blend-script") {
    throw "The pluginized image-blend package has a fixed manifest id; install a third-party package for custom ids."
}
$installer = Join-Path $PSScriptRoot "Install-LoomSampleArtPackage.ps1"
$arguments = @("-PackageName", "image-blend", "-BaseUrl", $BaseUrl, "-InstallMode", $InstallMode)
