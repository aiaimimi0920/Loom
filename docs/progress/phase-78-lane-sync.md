...
| --- | --- | --- | --- |
| A | F2, F3, F8 (minus the four Hook-side P2s), F9 | Loom `crates/**`, `framework-packages/**`, root `Cargo.toml` / `Cargo.lock`, Loom `.github/**`, `art-packages/samples/image-search/**`, `art-packages/shared/**`, `apps/desktop/**` | owns Loom `cargo` / `target/` |
| B | F4, F5, F6, F7, plus S1-2 (Hook half), S2-1, S2-2, S3-3, S3-4 | all of `Hook/`, Loom `art-packages/samples/stock-monitor/**`, `mcp-server-packages/**`, this file | Hook `npm` and Hook `src-tauri` cargo only |
